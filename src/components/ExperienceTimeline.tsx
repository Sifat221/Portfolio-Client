...
export const ExperienceTimeline: React.FC<ExperienceTimelineProps> = ({ experience }) => {
  return (
    <section id="experience" className="py-24 relative bg-[#0B101D]/80 border-t border-slate-800/80">
      <motion.div
        ...
        <div className="relative border-l-2 border-slate-800 ml-4 sm:ml-8 space-y-12 pl-6 sm:pl-10">
          {experience.map((exp, index) => (
            ...
            >
              ...
              <div className="absolute -left-[31px] sm:-left-[47px] top-1.5 w-10 h-10 rounded-xl bg-slate-900 border border-slate-700/80 text-[#9B8FCD] flex items-center justify-center group-hover:scale-110 group-hover:border-[#9B8FCD] group-hover:shadow-lg group-hover:shadow-[#9B8FCD]/20 transition-all duration-300">
                <Briefcase className="w-5 h-5" />
              </div>

              {/* Experience Card */}
              <div className="glass-card p-6 sm:p-8 rounded-3xl border border-slate-800/80 hover:border-[#9B8FCD]/50 transition-all duration-300 space-y-4">
                {/* Top Info */}
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-[#9B8FCD] font-medium font-mono">
                      {exp.company}
                    </p>
                  </div>
