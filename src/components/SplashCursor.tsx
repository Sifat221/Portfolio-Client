...
export const SplashCursor: React.FC<SplashCursorProps> = ({
  ...
  useEffect(() => {
    ...
    type PointerType = ReturnType<typeof Pointer>;
    ...
    const pointers: PointerType[] = [Pointer()];
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement) {
      ...
      if (!webglCtx) return { gl: null, ext: null };
      ...
      if (isWebGL2) {
        formatRGBA = getSupportedFormat(webglCtx, webglCtx.RGBA16F, webglCtx.RGBA, halfFloatTexType);
        formatRG = getSupportedFormat(webglCtx, webglCtx.RG16F, webglCtx.RG, halfFloatTexType);
        formatR = getSupportedFormat(webglCtx, webglCtx.R16F, webglCtx.RED, halfFloatTexType);
      } else {
        formatRGBA = getSupportedFormat(webglCtx, webglCtx.RGBA, webglCtx.RGBA, halfFloatTexType);
        formatRG = getSupportedFormat(webglCtx, webglCtx.RGBA, webglCtx.RGBA, halfFloatTexType);
        formatR = getSupportedFormat(webglCtx, webglCtx.RGBA, webglCtx.RGBA, halfFloatTexType);
      }
    ...
    function splatPointer(pointer: PointerType) {
      let dx = pointer.deltaX * config.SPLAT_FORCE;
      let dy = pointer.deltaY * config.SPLAT_FORCE;
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, pointer.color);
    }

    function clickSplat(pointer: PointerType) {
    ...
    function updatePointerMoveData(pointer: PointerType, posX: number, posY: number, color: { r: number; g: number; b: number }) {
      if (!canvas) return;
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width;
